		...
		if (KitAPI.getKit(p) == "Nenhum") {
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstace(), new Runnable() {
				...
				public void run() {
					...
					p.sendMessage("§c§lFly§7§lTrue §a: §7§lVoce entrou na arena Challenge!");
					BarAPI.setMessage(p, "§c§lFly§7§lTrue §a: §7§lVoce entrou na arena §c§lChallenge", 3);
			...
			if (args[0].equalsIgnoreCase("set")) {
			    if (p.hasPermission("fly.set")) {
			    	Set(p, "Challenge");
			    	p.sendMessage("§c§lFly§7§lTrue §a: §b§lVoce setou a arena Challenge!");
			}
		  }
		}
		return false;
